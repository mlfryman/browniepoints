...
describe('Prize', function(){
  ...
  beforeEach(function(done){
    cp.execFile(__dirname + '/../scripts/clean-db.sh', [db], {cwd:__dirname + '/../scripts'}, function(err, stdout, stderr){
      Prize.create(12, {owner_id:1,from_id:1,to_id:2,title:'a',description:'b',category_id:1,cost:10}, function(err, results){
        prizeId = results;
        done();
      });
    });
  ...
  describe('.nuke', function(){
    it('should nuke a prize', function(done){
      Prize.nuke(12, prizeId, function(err, pid){
        expect(err).to.be.null;
        expect(pid).to.equal(prizeId);
        done();
      });
    });
  });
  ...
  describe('.findAll', function(){
    it('should find all prizes for a Friendship', function(done){
      Prize.findAll(12, function(err, results){
        expect(err).to.be.null;
